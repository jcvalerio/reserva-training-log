...
4. **Progress** (`/progreso`) — three KPIs, weekly effective sets over four selectable periods, a front/back body map that filters related exercises (primary and secondary muscles), and weekly consistency. Exercise charts open on demand; completed sessions live at `/progreso/historial`. Measurements and tests remain at `/mediciones`. See [the September UX review](progreso-ux-review-2026-09.md) for the three competitor investigations and rationale for removing the competing summary sections.
