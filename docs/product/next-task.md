...
1. **A `painLocation` column on `setLog`.** Pain-by-joint is currently an inference, and the UI has to say so. This would make it a measurement, and it is the biggest honesty gap in the reporting.
