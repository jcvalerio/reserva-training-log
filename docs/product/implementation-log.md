...
**The editor was then redesigned off a screenshot, and the height was the smaller half of it.** Eleven denominations at a 52px pitch is **572px** — most of an 844px viewport — to ask about eight discs nobody had touched. Underneath that was a real bug: the list rendered `inventory` in stored order, and `gymInventorySchema` produces kg-family-then-lb-family, value-descending *within* each family. So `45 lb` (20.41 kg, the second-heaviest disc in the gym) rendered **sixth, below `5 kg`**, and the athlete scrolled past five plates they never touch to reach the one they always use. `normalize()` in plate-math and `sortHeaviestFirst` in plate-build were each already sorting by true mass inline; the editor was the one place that did not. Now one exported `byHeaviestFirst` in `units.ts`, used by all three — comparing printed numbers across unit families is the whole trap, since `45 > 5` says nothing until both are kilograms.

**The proposal was two columns (kg | lb) to halve the rows; the arithmetic kills it.** At ~326px of card content, two columns with an 8px gutter give 159px each. A stepper cluster is 44 + digit + 44 + gaps ≈ 114px, leaving ~45px for a label that needs 52–56px for `2.5 lb` — failing before row padding, and worse under iOS accessibility text sizing, where `text-sm` grows with the root font while the 44px buttons cannot shrink. This card has already shipped and fixed one 20px tap target; it is not the place to shave another. It also optimises the wrong axis: two columns still render all eleven denominations on every visit, and the complaint was scrolling past discs you do not use.
...
**The complaint, and it was accurate.** Users return to the hip thrust, read "122 kg", and their gym stocks discs in pounds — so before they can lift they convert and hunt for a combination that sums to it. They had been recording that working-out in `setLog.notes`, and losing it: `getPreviousExercisePerformance` is `.limit(1)`, so a note written in week one is visible in week two and gone by week three. Both halves of that were checked against the code before anything was designed. Worth noting, since three of the last four issues had premises that fell apart on inspection.

**The convention was measured, not chosen, and that inverted the design.** Asked what they actually type, the answer was: plate-loaded lifts as **total discs, both sides, bar excluded** (122 for 6 × 45 lb — the bar would make it ~142), and dumbbells as **one** dumbbell (15 for a 15 kg dumbbell in each hand). The architecture had assumed bar-inclusive on the grounds that "that is how every logging app talks" — an appeal to convention in a design whose whole thesis is *do not reinterpret what the athletes actually did*. Measuring first killed a tare model, a `load_entry` column and two nullable unit columns before any of them were written.

**Nothing reinterprets a logged weight** — now invariant 13 in `data-model.md`. Doubling per-side loads or adding the bar would move every historical `/progreso` number, every PR, every improvement signal (5% threshold, so every comparison spanning the change fires spuriously) and, decisively, `buildWeeklyLoadGuardrail`, whose 1.3 ratio would see a 2.0 and silently suppress earned increases for weeks. A display convenience must not be able to trip a safety brake.

**Minimum change, not minimum plates.** This was the design error the whole feature turned on, and it survived two rounds of reasoning before real numbers killed it. Snapping to the nearest buildable total and rendering its fewest-disc recipe prescribes, for a +5% move off 122.47 kg, `1×45lb + 1×35lb + 1×25lb + 1×15kg` per side — a full rebuild to add three kilos. So the primary output is an instruction relative to the logged weight: **"Añade 1 × 5 lb + 1 × 2.5 lb por lado → 129.3 kg"**. The mass delta of adding a pair is base-independent, which is why this needs no record of what is currently on the bar — and that observation removed the schema question entirely.

**The same error, one level down, found by testing rather than by thinking.** `buildFromScratch(122)` returned 122.06 kg as `2×45lb + 2×10lb + 1×2.5lb + 1×10kg` — six discs across four denominations, to be 0.06 kg nearer than `3 × 45 lb` at 122.47, which is what was actually on the bar. Then a third term was needed: plate-count-then-distance still picked `1×25kg + 1×20kg + 1×35lb` over `3 × 45 lb`. The objective is fewest discs, then fewest *distinct* denominations, then closest. Both are pinned.

**The band became a range.** `progression-rules.md` has always specified 5–10% and 2.5–5%; the code collapsed each to its low end and rounded to 0.5 kg. Fine while any weight was reachable, useless against a discrete set of discs. The low end stays the target — `suggestNextWeightKg` returns byte-identical values, and the existing tests pin that — and `high` only bounds what else may win. Selection is closest-to-target, then fewest discs; **that order is load-bearing**, because disc-count-first picks +7.4% over +5.6% on the real numbers, and paying two percent of load for one fewer disc is the wrong trade on a hinge for a 47-year-old.

**Issue #5, caught live rather than argued.** On real data an exercise at 20 kg earned an increase and the app suggested **21 kg** — unbuildable, since the smallest pair this gym stocks is 2.5 lb (1.13 kg) and the next rung is 22.27 kg at +11.3%, outside the band. The runner now says there is no appropriate load jump and to add a rep. That is the shape #5 should be re-scoped to: *suggest a load change the athlete can actually perform, or suggest reps instead* — which subsumes impossible, impractical and too-large. Mixed-unit inventories weaken the *impossible* case (70 buildable rungs between 120 and 132 kg) and strengthen the *impractical* one.

**Placement.** `athlete_gym` holds the rack because the inventory is a property of the room — per-exercise, an athlete retypes it for every movement and the copies drift. It could not go on the `exercise` catalog, whose NULL-profile rows are shared by every profile in the product; that was issue #5's own proposal. `exercise_setup` is keyed on the **normalized exercise name**, not `exerciseId`, because `createSubstitutePrescription` writes `exerciseId: null` whenever a typed name matches no catalog entry — exactly the case that most needs a setup note. Not on `exercisePrescription`, because `redeemPlanShare` clones every prescription column **into another human's account**.

**Cost on the hottest screen.** Two queries for the whole session, not two per exercise — `getSessionRunDetails` already runs an N+1 there. The enumeration (up to ~75k multisets) runs on the server and is memoised per inventory, since the achievable set belongs to the gym rather than the movement; the runner receives a plain instruction object.

**Verified in a browser at 390×844**, on real data: the inventory round-trips (`45, 35, 25, 10, 5, 2.5` lb and `25, 20, 15, 10, 5` kg), the check line reads "60 kg se arma con 1 × 25 kg + 1 × 5 kg por lado", a 40 kg lift renders "La vez pasada: 1 × 20 kg por lado", and the 20 kg case produces the no-appropriate-jump fallback. No horizontal overflow anywhere; content `scrollWidth` 390. Two defects were found by measuring rather than reading: the "Armar desde cero" `<summary>` was a **20px** tap target where the neighbouring `<summary>` on the same card is 44px, and the rendered sentence mixed decimal conventions — `1 × 2,5 lb` beside `54.5kg` — because `formatKg` is dot-decimal app-wide and the new formatter was comma-decimal. Both fixed. The instruction line itself was measured by temporarily forcing it on and reverting, the same technique the weekly-load badge used.

**Chrome's minimum window width is 500px**, so the 390 pass constrains the document instead. Faithful here since no Tailwind breakpoint falls between 390 and 500, but it is not a true 390 viewport.

**Known gaps.** The panel renders only where there is a previous performance, so a first-ever session on an exercise gets nothing — acceptable, since the stated pain is returning to a lift, but it also means "¿lleva discos?" cannot be answered until the second session. Stacks and fixed dumbbells have their columns (`increment_*`, `add_on_*`) and no consumer yet — the uniform-increment grid is the next slice, and until it lands `suggestNextWeightKg` still rounds to 0.5 kg for them. Reductions render a from-scratch recipe rather than a removal instruction, because removing a disc needs to know what is on the bar and nothing persists that yet. Nobody has used this in a real session.
...
Status: committed as `c70a942` on `main`. `lint`/`typecheck`/`test` (543 passing, +5 net)/`build` all green. No migration.
...
Status: committed as `18edd45` on `main`, **not yet deployed**. `lint`/`typecheck`/`test` (538 passing, +15 new)/`build` all green. No migration.
...
Status: committed as `5408da2` on `main`, **not yet deployed**. `lint`/`typecheck`/`test` (523 passing, +20 new)/`build` all green. No migration.
...
Status: shipped. Committed as `9e5d241` on `main`, `lint`/`typecheck`/`test` (456 passing, +7 new)/`build` all green. Deployed via `npx vercel deploy --prod --yes` (`dpl_APt3JCTV4SUoiyaJxGdiHT7FEpe3`, aliased to `gym.jcvalerio.com`; `/` 200 confirmed live). No migration. No active session at deploy time.
...
Status: shipped. Committed as `b4b41ad` on `main`, `lint`/`typecheck`/`test` (447 passing)/`build` all green. Deployed via `npx vercel deploy --prod --yes` (`dpl_8FqU21vePGR9MF7HCsPhKW3zTuuX`, aliased to `gym.jcvalerio.com`; `/` 200 confirmed live). No migration. No active session at deploy time.
...
Status: shipped. Committed as `8c36f82` on `main`, `lint`/`typecheck`/`test` (447 passing)/`build` all green. Deployed via `npx vercel deploy --prod --yes` (`dpl_G19xi7v76ddiXeuggtxoFnRCT5AW`, aliased to `gym.jcvalerio.com`; `/` 200 confirmed live). No migration. No active session at deploy time.
...
Status: shipped. Committed as `2607a00` on `main`, `lint`/`typecheck`/`test` (447 passing)/`build` all green. Deployed via `npx vercel deploy --prod --yes` (`dpl_EwaRj5bLALJusAX6HFMnWEYboUnM`, aliased to `gym.jcvalerio.com`; `/` 200 confirmed live). No migration. No active session at deploy time.
...
Status: shipped. Committed as `e24e509` on `main`, `lint`/`typecheck`/`test` (447 passing)/`build` all green. Deployed via `npx vercel deploy --prod --yes` (`dpl_B69VXVmxRCBrvuG2n9bqBfCgAyvT`, aliased to `gym.jcvalerio.com`; `/` 200, `/plan/builder` 307-to-auth confirmed live). No migration. No active session at deploy time.
...
Status: shipped. Committed as `5defeb0` on `main`, `lint`/`typecheck`/`test` (447 passing)/`build` all green. Deployed via `npx vercel deploy --prod --yes` (`dpl_G6vcdsqujYd51X2GjdMLFiLYKxjK`, aliased to `gym.jcvalerio.com`; `/` 200, `/plan/builder` 307-to-auth confirmed live). No migration. No active session at deploy time.
...
This was a latent gap, not something newly broken — it needed both a real athlete with logged history *and* someone editing that specific session's exercise list to surface, which is exactly what `574710e` (unblocking `/plan` → `/plan/builder` for an already-active plan) made possible for the first time.
...
Status: shipped. Committed as `405e01d` on `main`, `lint`/`typecheck`/`test` (443 passing, +2 new)/`build` all green. Deployed together with the `/entrenar` fixes above via the same `npx vercel deploy --prod --yes`. No migration — presentation only.
...
Before trimming, every commit referenced by a removed "Prior status" section was verified to appear in the implementation log (`99e7a1e`, `6623f4c`, `3f0dc32`, `6dbd3ba`, `74c91a3`, `b33bf32`, `466ebbf`, `2c49b72` — all present). Nothing was lost; the duplication was.
...
Status: shipped. Committed as `f2af442` on `main` (`feat: add a "Semana pasada" period to Series por grupo muscular`), `lint`/`typecheck`/`test` (429 passing, +4 new)/`build` all green. Deployed via `npx vercel deploy --prod --yes`; live (`/` and `/guia` 200, `/progreso` and `/entrenar` 307-to-auth). No migration, no data change.
...
Status: shipped. Committed as `55a2da4` on `main` (`feat: swap an exercise mid-session, and raise the reading type scale`) — 17 files, bundling both undeployed pieces into one commit per this project's established pattern for stacked work. 1Password's SSH signing agent worked first try. Deployed to production via `npx vercel deploy --prod --yes` (live at `https://gym.jcvalerio.com`; `/` and `/guia` HTTP 200, `/entrenar` and `/plan/rutina` HTTP 307-to-auth confirmed).
...
Status: shipped. Committed as `97f6921` on `main` (`feat: let a logged set be corrected or deleted, and unblock iOS text zoom`) — 22 files, both of this session's pieces in one commit per this project's established pattern for stacked work. 1Password's SSH signing agent worked first try this time (no retry needed, unlike the last two sessions). Deployed to production via `npx vercel deploy --prod --yes` (live at `https://gym.jcvalerio.com`, aliased successfully; `/` and `/guia` HTTP 200, `/entrenar` and `/plan/rutina` HTTP 307-to-auth confirmed).
...
Status: shipped. Committed as `99e7a1e` on `main` (`feat: add a minimalistic "Ver técnica en YouTube" tap target`). Deployed to production via `npx vercel deploy --prod --yes` (live at `https://gym.jcvalerio.com`, aliased successfully; `/` and `/guia` HTTP 200, `/entrenar` and `/plan/rutina` HTTP 307-to-auth confirmed; no schema change, nothing for the automatic migration step to do). Confirmed no workout session was active at deploy time via a direct DB query (using the corrected `grep '^DATABASE_URL='` pattern, not the earlier flawed one — see below).
...
Status: shipped. Committed as `6623f4c` on `main` (`feat: let a set exceed the plan, and add a RIR/AMRAP/progression guide`) — bundled both of this session's undeployed pieces into one commit, per this project's established pattern for stacked work. The first commit attempt failed with 1Password's SSH signing agent ("failed to fill whole buffer" — locked), succeeded on retry once unlocked, same failure mode this project has hit before. Deployed to production via `npx vercel deploy --prod --yes` (live at `https://gym.jcvalerio.com`, aliased successfully; `/` and `/guia` HTTP 200, `/entrenar` HTTP 307-to-auth confirmed; no schema change, nothing for the automatic migration step to do). Confirmed no workout session was active at deploy time via a direct DB query — the account's only recent session was already `completed`.
...
Status: shipped. Committed as `3f0dc32` on `main` (`fix: standardize back-navigation to one AppShell-owned pattern`) — the first commit this session that needed a retry: 1Password's SSH commit-signing agent failed once with "failed to fill whole buffer" (likely locked), succeeded on retry after the user unlocked it. Deployed to production via `npx vercel deploy --prod --yes` (live at `https://gym.jcvalerio.com`, aliased successfully; `/` HTTP 200 and `/mediciones` HTTP 307-to-auth confirmed; no schema change, nothing for the automatic migration step to do).
...
Status: shipped. Committed as `6dbd3ba` on `main` (`feat: add "Readaptación" template from a user-supplied infographic`). Deployed to production via `npx vercel deploy --prod --yes` (live at `https://gym.jcvalerio.com`, aliased successfully; `/` HTTP 200 and `/plan/templates` HTTP 307-to-auth confirmed; no schema change, nothing for the automatic migration step to do).
...
Status: shipped. Committed as `3aa0475` on `main` ("fix: round-2 mobile UX audit — Home stale state, sticky-button overlap, builder gaps, a11y"), then deployed to production via `npx vercel deploy --prod --yes` (live at `https://gym.jcvalerio.com`, HTTP 200 verified, no migration involved since this round had no schema changes).
...
Status: shipped. Deployed to production via `npx vercel deploy --prod --yes` (live at `https://gym.jcvalerio.com`, HTTP 200 verified, migration step ran with zero diff since this was a UI-only change), then committed locally as `e1949cb` on `main` ("fix: mobile UI/UX audit — button-contrast bug, /plan scroll, nav cleanup").
