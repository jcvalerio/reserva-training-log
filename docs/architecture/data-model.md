...
Fields: `id`, `athleteProfileId`, `gymId` (FK → `AthleteGym`, cascade), `exerciseKey`, `exerciseId` (nullable FK → `Exercise`, set null), `setupNotesEs`, `loadingModel`, `incrementValue`/`incrementUnit`, `addOnValue`/`addOnUnit`, `plateInventory` (nullable override), `createdAt`, `updatedAt`. Unique on `(athleteProfileId, gymId, exerciseKey)`.
