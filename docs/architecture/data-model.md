...
Fields: `id`, `exerciseLogId`, `setNumber`, `side` (`bilateral | left | right`, default bilateral), `actualWeightKg`/`actualReps`/`rir` (nullable — null for a duration-type set), `actualDurationSeconds` (nullable — null for a strength-type set; exactly one of these two groups is populated, enforced at the Zod layer via a discriminated union on the sibling prescription's `prescriptionType`, not a DB constraint), `painScore` (**nullable since 2026-08-31**, and the null means *not asked* — see note 10), `painLocation` (nullable), `notes`, `completedAt`.
...
Added 2026-09-06 (migration `0026`). A room the athlete trains in, and what it stocks.

Fields: `id`, `athleteProfileId` (FK → `AthleteProfile`, cascade), `nameEs`, `isDefault`, `displayUnit` (`kg | lb`), `plateInventory` (jsonb `[{ value, unit }]`), `createdAt`, `updatedAt`.

- **The plate inventory belongs to the room, not the exercise.** Every plate-loaded movement in a building draws on the same rack; stored per exercise, an athlete retypes it for each movement and the copies drift until one exercise believes 45 lb discs exist and another does not.
- **It cannot go on the `Exercise` catalog.** A catalog row with `athleteProfileId IS NULL` is shared by every profile in the product, so "entered once per gym" would be entered once for everyone. Issue #5 proposed exactly that placement; this is why it was not taken.
- **A denomination stores the number printed on the disc plus its unit**, never a pre-converted kilogram value — otherwise "45 lb" can never be rendered back, and rendering it back is the entire feature. Conversion is exact (`0.45359237`) and rounds only at the 2 dp edge.
- **No per-denomination counts, deliberately.** Tracking how many of each disc a gym owns makes every answer depend on state nobody maintains, and a calculator that is confidently wrong is worse than none. The enumeration is bounded by constants in `plate-math.ts` instead.
- **Rows are created lazily in application code, never by a migration.** A migration that INSERTs one row per `athlete_profile` can fail mid-`vercel build` and take the deploy down. `getOrCreateDefaultGym` seeds the name from `athleteProfile.gymContext`, which had been written and re-displayed since M0 and read by nothing — this is its first reader.
- `displayUnit` is display only. It never changes what is stored.

## ExerciseSetup

Added 2026-09-06 (migration `0026`). Durable facts about one athlete's use of one exercise at one gym.

Fields: `id`, `athleteProfileId`, `gymId` (FK → `AthleteGym`, cascade), `exerciseKey`, `exerciseId` (nullable FK → `Exercise`, set null), `setupNotesEs`, `loadingModel`, `incrementValue`/`incrementUnit`, `addOnValue`/`addOnUnit`, `plateInventory` (nullable override), `plateBuild`/`plateBuildTotalKg`/`plateBuildRecordedAt`, `createdAt`, `updatedAt`. Unique on `(athleteProfileId, gymId, exerciseKey)` — which is also the lookup index for `(athleteProfileId, gymId)`, since a b-tree serves any leftmost prefix; the separate two-column index was dropped in `0028`.

- **Rows are written only by actions, never by a render.** `getDefaultGym` reads and may return null; `getOrCreateDefaultGym` is for write paths only. A page that creates a row on GET does it during builds and prefetches too, and there is nothing to create until the athlete has actually answered something.

- **`exerciseKey` is the normalized `exerciseNameEs`, not `exerciseId`.** `exerciseId` is nullable and `createSubstitutePrescription` writes null whenever a typed name matches no catalog entry — which is exactly the case that most needs a setup note, since an unfamiliar machine is why you substituted. Keying on the name also means setup and history decay together, since `getPreviousExercisePerformance` matches on the name too. Reuses `normalizeExerciseName`; do not write a second normalizer.
- **Not on `ExercisePrescription`**, for three reasons that each cost real data: `redeemPlanShare` clones every prescription column into another human's account; `createSubstitutePrescription` inherits dosage onto a *different* machine, where a seat height is actively wrong; and the same exercise on three session templates means three copies that drift. (It does survive the ordinary edit→activate cycle — `revertActivePlanToDraft` flips status in place — so that is not the reason.)
- **`loadingModel` is load-bearing, not descriptive.** It is the only thing separating a selectorized stack from a plate-loaded machine; `exercisePrescription.loadMechanism = "machine"` covers both and nothing else stored can tell them apart. It is never inferred — guessing renders a disc recipe for a pin stack.
- `setupNotesEs` is free text in v1 on purpose. Seat height, pin position and grip width vary per manufacturer, and five labelled numeric fields on a 390px screen at this app's raised type scale is worse than one textarea. Structure what recurs, later.
...
9. `painLocation` is the reported truth; the joints an exercise loads are only a fallback for sets logged before the column existed (added 2026-08-09). The two must never be presented identically — an inferred location would report "hombro" for a hurting wrist. Since 2026-08-31 it **does** affect the progression thresholds: `muscular` no longer forces a load reduction, while a missing location is treated as joint pain. A tenth value, `neural`, was added 2026-09-06 (migration `0025`) for radiating pain, tingling and numbness; it escalates on presence rather than on the score, and the inference fallback can never produce it, so a `neural` row is always something the athlete reported.
10. `setLog.painScore` has three states and they are not interchangeable: `null` = never asked, `0` = asked and nothing hurt, `1-10` = reported. Pain is asked **once per exercise**, not once per set, so on a normal exercise exactly one set carries a value and the rest are null. The answer is attached to the exercise's last set (`recordExercisePain`), which keeps one answer as one row — copying it across every set would multiply one report into three in the pain-by-location report. Readers must skip nulls **explicitly**: `Math.max(...[null])` is `0` and `null <= 0` is `true`, so a coercion looks correct while turning "not asked" into "no pain", and one of those readers (`suggestProgression`) decides how much weight someone loads. Rows predating the change also hold `0` and are deliberately not backfilled — `0` blocks nothing, so reading them as "no pain" changes no behaviour.
11. Effective sets for a unilateral exercise are `max(left, right) + bilateral`, never a distinct-`setNumber` count: `setNumber` is assigned across the whole exercise log regardless of side, so 3 left + 3 right carries setNumbers 1–6 and a distinct count would double it.
12. At most one `WorkoutPlan` per profile has `status = 'active'` (DB-enforced via a partial unique index); activating a new one archives whichever plan was active, it doesn't delete it.
13. **`setLog.actualWeightKg` is the number the athlete typed, and nothing reinterprets it.** The convention was measured, not chosen (2026-09-06): for a plate-loaded implement it is the total mass of the discs across **both sides, excluding the bar** — a hip thrust with 6 × 45 lb is logged 122, not 142 with the bar and not 61 per side. For a fixed dumbbell it is **one** dumbbell — a hammer curl with 15 kg in each hand is logged 15, not 30. For a stack it is the marked value. `AthleteGym`/`ExerciseSetup` describe how to help the athlete *arrive* at that number; they never transform it on write or reinterpret it on read.

    This one matters more than it looks. Doubling per-side loads, or adding the bar, would move every historical `/progreso` number, every personal record, every improvement signal (which uses a **5% threshold**, so every comparison spanning the change fires spuriously) and — decisively — `buildWeeklyLoadGuardrail`, whose **1.3** ratio would see a 2.0 and silently suppress every earned load increase across that muscle group for weeks. A display convenience must not be able to trip a safety brake. There is deliberately no stored per-exercise override of the convention: a nullable column nobody sets is the `limitation.requiresPainTracking` failure (issue #14, open), so the convention is stated to the athlete **in words** in the runner instead, which is the cheaper detection mechanism.

14. **A plate build is recorded, never derived.** `exerciseSetup.plateBuild` holds the discs the athlete says are on the bar (per side, with `plateBuildTotalKg` across both and `plateBuildRecordedAt` for its age). Everything else in the plate assistant is arithmetic; this is the one fact, and only the athlete can supply it.

    Learned by shipping the opposite, 2026-09-07. The first version answered "armar desde cero" with `buildFromScratch`'s recipe under the heading **"La vez pasada"**, and printed that recipe's mass in parentheses marked **"reales"** — two assertions about the athlete's history from a computation that had never seen a bar. On preview data it claimed `1 × 20 kg + 1 × 25 lb (62.7 kg reales)` for a 63 kg lift the athlete had loaded with 45 lb discs, because the 25 kg plates live at the far end of the gym. Walking distance is not in the model and never will be, so no better objective function fixes this: the rule is that **a derived build is offered in the conditional and claims no mass, and only a recorded build may state one.**

    Consequences to hold: a recorded build is the truer base for the *plate* arithmetic (a step off 3 × 45 lb is computed from 122.47, not from the 122 someone typed), but it never rebases the progression itself — `suggestNextWeightKg` still reads the logged weight, so no stored number and no guardrail ever sees a value it did not see before. It reaches `setLog` only as a **prefill on the next set**, which is invariant 13 above. A build whose mass no longer matches the last logged weight is marked stale and stops prefilling, but is kept: the denominations an athlete reaches for outlive any particular load.
